...
public class ArrayManipulation
{
    public static EDGE[] RemoveEdges(EDGE[] listEdges, EDGE edgeRemoved)
    {
        ...
        {
            if (edge.Vertex()[0] == edgeRemoved.Vertex()[0] && edge.Vertex()[1] == edgeRemoved.Vertex()[1] && edge.EdgeWeight() == edgeRemoved.EdgeWeight()) continue;
            result.Add(edge);
        }
    ...
    public static EDGE[] AddEdges(EDGE[] listEdge, EDGE edgeAdd)
    {
        ...
        for (var i = 0; i < listEdge.Length; i++) newlistEdges[i] = listEdge[i];
    ...
    {
        var total = 0;
        foreach (var edge in listEdges) total += edge.EdgeWeight();
    ...
    public static int[] Addelement(int[] currentArray, int newelement)
    {
        ...
        for (var i = 0; i < currentArray.Length; i++) newlistEdges[i] = currentArray[i];
