...
public class AdjacencyMatrix
{
    ...
    {
        readAdjacencyMatrix(path);
        IsUndirectedGraph();
        countDegrees();
        ConvertMatrixToAdjacencylist();
        ConvertMatrixtoEDGE();
        Console.WriteLine("List edge before short");
        showEDGElist();
        listEDGE = Sort.MergeSort(listEDGE);
        Console.WriteLine("After MergSort");
        showEDGElist();
    }
    ...
    private void showEDGElist()
    {
        ...
        {
            Console.WriteLine($"Canh:{edge.Vertex()[0]}-{edge.Vertex()[1]}|Trong so:{edge.EdgeWeight()}");
        }
    }

    private void ConvertMatrixtoEDGE()
    {
        var importEdges = new List<EDGE>();
        switch (Undirect)
        {
            case false:
