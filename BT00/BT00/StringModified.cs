...
public static class StringModified
{
    public static Date InputDate()
    {
        ...
        // }
        var month = Inputnumber("Tháng", 1, 12);
...
//cố định giá trị string được nhập vào
    ...
    public static int Inputnumber(string functionName, int min, int max)
    {
        ...
        var check = int.TryParse(input, out var value);
    ...
    {
        Date exp;
        var expmonth = Inputnumber("Số tháng sử dụng", 1, 100);
        var totalmonth = expmonth + mfg.Month;
    ...
    {
        for (var i = 0; i < array.Length; i++) Console.WriteLine($"{i + 1}. {array[i]}");
    }
}